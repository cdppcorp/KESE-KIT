...
터미널(CMD, PowerShell, bash 등)에서 실행:
...
> v2.x에서 업그레이드하는 경우:
> ```bash
> claude plugins uninstall kesekit
> claude plugins install kesekit
> ```

### 2단계: Claude Code에서 사용

Claude Code를 실행한 뒤, 프롬프트에 슬래시 명령어를 입력합니다:
...
v3.0에서 모든 스킬이 단일 `kesekit` 네임스페이스로 통합되었습니다. v2.x 이하를 사용하셨다면 다음 명령어 변경사항을 확인하세요:
...
Run in your terminal (CMD, PowerShell, bash, etc.):
...
Launch Claude Code, then type slash commands at the prompt:
